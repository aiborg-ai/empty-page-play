-- Safe Initial CMS Schema Migration
...
CREATE INDEX IF NOT EXISTS idx_contents_search_vector ON public.contents USING gin(search_vector);
...
CREATE INDEX IF NOT EXISTS idx_ai_agents_project_id ON public.ai_agents(project_id);
...
-- Full-text search trigger for contents (create or replace to handle existing)
...
DROP TRIGGER IF EXISTS update_contents_search_vector ON public.contents;
CREATE TRIGGER update_contents_search_vector
    BEFORE INSERT OR UPDATE ON public.contents
    FOR EACH ROW
    EXECUTE FUNCTION update_search_vector();

-- Update timestamp triggers (create or replace to handle existing)
...
DROP TRIGGER IF EXISTS update_content_types_updated_at ON public.content_types;
